...
public class ShepherdToLibrarianDistributionGoal extends AbstractInventoryDistributionGoal {
    private static final double RECIPIENT_SCAN_RANGE = 24.0D;
    private static final Set<Item> COLLECTED_MATERIALS = Set.of(
            Items.STRING
    );

    public ShepherdToLibrarianDistributionGoal(VillagerEntity villager, BlockPos jobPos, BlockPos chestPos, BlockPos craftingTablePos) {
        super(villager, jobPos, chestPos, craftingTablePos);
    }

    @Override
    protected boolean isDistributableItem(ItemStack stack) {
        return !stack.isEmpty() && (stack.isIn(ItemTags.WOOL) || COLLECTED_MATERIALS.contains(stack.getItem()));
    }
