...
public class LibrarianCraftingGoal extends Goal {
    ...
    public boolean canStart() {
        ...
        CraftingCheckLogger.report(world, "Librarian", formatCheckResult(lastCheckCount));
    ...
    private void craftOnce(ServerWorld world) {
        ...
        List<LibrarianRecipe> craftable = getCraftableRecipes(world, inventory);
        if (craftable.isEmpty()) {
            return;
        }

        LibrarianRecipe recipe = craftable.get(villager.getRandom().nextInt(craftable.size()));
    ...
    private List<LibrarianRecipe> getCraftableRecipes(ServerWorld world, Inventory inventory) {
        List<LibrarianRecipe> recipes = new ArrayList<>();
        for (RecipeEntry<CraftingRecipe> entry : world.getRecipeManager().listAllOfType(RecipeType.CRAFTING)) {
            ...
            if (!isLibrarianOutput(result)) {
                continue;
            }
            if (canCraft(inventory, recipe)) {
                recipes.add(new LibrarianRecipe(recipe, result));
            }
        }
        return recipes;
    }

    private boolean isLibrarianOutput(ItemStack stack) {
        if (stack.isEmpty()) {
            return false;
        }
        // "Written book" crafting is treated as a writable book (book & quill) because vanilla crafting cannot create a signed book.
    ...
    private record LibrarianRecipe(CraftingRecipe recipe, ItemStack output) {
    }

    private String formatCheckResult(int craftableCount) {
        if (craftableCount == 1) {
            return "1 librarian item available to craft";
        }
        return craftableCount + " librarian items available to craft";
    }

    private String formatCraftedResult(int craftableCount, ItemStack crafted) {
        String craftedName = crafted.getName().getString();
        if (craftableCount == 1) {
            return "1 librarian item available to craft - 1 " + craftedName + " crafted";
        }
        return craftableCount + " librarian items available to craft - 1 " + craftedName + " crafted";
    }
}
