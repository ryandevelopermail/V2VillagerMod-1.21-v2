...
public class ClericCraftingGoal extends Goal {
    ...
    public ClericCraftingGoal(VillagerEntity villager, BlockPos jobPos, BlockPos chestPos, @Nullable BlockPos craftingTablePos) {
    ...
    public boolean canStart() {
        ...
        CraftingCheckLogger.report(world, "Cleric", formatCheckResult(lastCheckCount));
    ...
    private void craftOnce(ServerWorld world) {
        ...
        List<ClericRecipe> craftable = getCraftableRecipes(world, inventory);
        if (craftable.isEmpty()) {
            return;
        }

        ClericRecipe recipe = craftable.get(villager.getRandom().nextInt(craftable.size()));
    ...
    private List<ClericRecipe> getCraftableRecipes(ServerWorld world, Inventory inventory) {
        List<ClericRecipe> recipes = new ArrayList<>();
        for (RecipeEntry<CraftingRecipe> entry : world.getRecipeManager().listAllOfType(RecipeType.CRAFTING)) {
            ...
            if (canCraft(inventory, recipe)) {
                recipes.add(new ClericRecipe(recipe, result));
            }
        }
        return recipes;
    }
    ...
    private record ClericRecipe(CraftingRecipe recipe, ItemStack output) {
    }

    private String formatCheckResult(int craftableCount) {
        if (craftableCount == 1) {
            return "1 recipe available to craft";
        }
        return craftableCount + " recipes available to craft";
    }

    private String formatCraftedResult(int craftableCount, ItemStack crafted) {
        String craftedName = crafted.getName().getString();
        if (craftableCount == 1) {
            return "1 recipe available to craft - 1 " + craftedName + " crafted";
        }
        return craftableCount + " recipes available to craft - 1 " + craftedName + " crafted";
    }
}
