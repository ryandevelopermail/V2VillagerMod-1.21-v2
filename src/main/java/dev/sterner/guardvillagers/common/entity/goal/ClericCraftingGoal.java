...
public class ClericCraftingGoal extends Goal {
    ...
    public ClericCraftingGoal(VillagerEntity villager, BlockPos jobPos, BlockPos chestPos, BlockPos craftingTablePos) {
    ...
    public boolean canStart() {
        ...
        CraftingCheckLogger.report(villager, "Cleric", immediateCheckPending ? "immediate request" : "natural interval", formatCheckResult(lastCheckCount));
    ...
    private void craftOnce(ServerWorld world) {
        ...
        if (consumeIngredients(inventory, recipe.requirements)) {
            ...
            inventory.markDirty();
            craftedToday++;
            CraftingCheckLogger.report(villager, "Cleric", formatCraftedResult(lastCheckCount, recipe.output));
        }
    }
    ...
    private enum Recipe {
        BREWING_STAND(new ItemStack(Items.BREWING_STAND),
                new IngredientRequirement(stack -> stack.isOf(Items.BLAZE_ROD), 1),
